#!/usr/bin/env ruby
# translate_songs.rb - Automated Tatar to Russian lyrics translation using Codex
...
class SongTranslator
  ...
  def get_translations(batch_data)
    # Prepare Codex command with JSON data directly in the prompt
    ...
    begin
      ...
      if status.success?
        ...
        if json_match
          ...
          puts "Warning: Could not find JSON in Codex response"
          puts "Response: #{stdout[0..500]}..." if stdout.length > 500
        end
      else
        puts "Error running Codex: #{stderr}"
      end
